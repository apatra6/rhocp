...
registerMooseObject("RhocpApp", DDCPTSTStressUpdate);

InputParameters
DDCPTSTStressUpdate::validParams()
{
  InputParameters params = ComputeStressBase::validParams();
  params.addClassDescription("Stress calculation using the Visco-Plsatic Crystal Plasticity Material");
...
DDCPTSTStressUpdate::DDCPTSTStressUpdate(const InputParameters & parameters) :
...
DDCPTSTStressUpdate::~DDCPTSTStressUpdate()
{
}

void DDCPTSTStressUpdate::initQpStatefulProperties()
{
...
void DDCPTSTStressUpdate::computeQpStress()
{
  ...
  for(unsigned int k = 0; k < _num_slip_sys; k++) {
    Real refgdg = rho_m[k]*b_mag*d_disl[k]*attempt_freq;
    Real tau_c = s_a[k] + s_t[k];
    // if((abs(tau_eff[k]) < tau_c) && (abs(tau_eff[k]) > 0.5*tau_c)) {
    if(abs(tau_eff[k]) < tau_c) {
      gamma_dot_g[k] = refgdg*exp((-delF0/B_k/_temp[_qp])*power(1 - power(abs(tau_eff[k])/tau_c, p), q))*sgn(tau_eff[k]);
    }
    else if(abs(tau_eff[k]) >= tau_c) {
      gamma_dot_g[k] = refgdg*sgn(tau_eff[k]);
    }
...
//       }
    ...
    for(unsigned int ia = 0; ia < _num_slip_sys; ia++) {
      ...
      Real tau_c = s_a[ia] + s_t[ia];

      for(unsigned int ib = 0; ib < _num_slip_sys; ib++) {
        // if ((abs(tau_eff[ia]) < tau_c) && (abs(tau_eff[ia]) > 0.5*tau_c)) {
        if (abs(tau_eff[ia]) < tau_c) {
          array3[ia][ib] = -gamma_dot_g[ia]*(p*q*(delF0/B_k/_temp[_qp])*power(1.e0 - power(abs(tau_eff[ia])/tau_c, p), q - 1)*power(abs(tau_eff[ia])/tau_c, p - 1))*
          ((dTaudGd[ia][ib] - dbsdgb[ia][ib])*sgn(tau_eff[ia])/tau_c - abs(tau_eff[ia])*(dsadgb[ia][ib] + dstdgb[ia][ib])/(tau_c*tau_c)) -drhomdgb[ia][ib]*gamma_dot_g[ia]/rho_m[ia];
        }
        else if(abs(tau_eff[ia]) >= tau_c) {
          array3[ia][ib] = 0.0;
        }
      }
      ...
      for (unsigned int k = 0; k < 3; k++) {
        for (unsigned int l = 0; l < 3; l++) {
          for(unsigned int n = 0; n < _num_slip_sys; n++) {
            ...
            Real tau_c = s_a[n] + s_t[n];

            // if ((abs(tau_eff[n]) < tau_c) && (abs(tau_eff[n]) > 0.5*tau_c)) {
            if (abs(tau_eff[n]) < tau_c) {
              ddpdsig[i][j][k][l] = ddpdsig[i][j][k][l] + (xs[i][n]*xm[j][n] + xm[i][n]*xs[j][n])*(((xs[k][n]*xm[l][n] + xm[k][n]*xs[l][n])*(gamma_dot_g[n]*(p*q*(delF0/B_k/_temp[_qp])*power(1.0 - power(abs(tau_eff[n])/tau_c, p), q - 1.0)*power(abs(tau_eff[n])/tau_c, p - 1.0)*sgn(tau_eff[n])/tau_c))));
            }
            else if(abs(tau_eff[n]) >= tau_c) {
              ddpdsig[i][j][k][l] = ddpdsig[i][j][k][l] + (xs[i][n]*xm[j][n] + xm[i][n]*xs[j][n])*(xs[k][n]*xm[l][n] + xm[k][n]*xs[l][n]);
            }
          }
        }
      }
...
//   }
  ...
  // Store mobile dislocation density values SDV 50 + _num_slip_sys
  for (unsigned int i = 0; i < _num_slip_sys; i++) {
    n = n + 1;
    _state_var[_qp][n] = rho_m[i];
  }

  // Store immobile dislocation density values SDV 50 + _num_slip_sys*2
  for (unsigned int i = 0; i < _num_slip_sys; i++) {
    n = n + 1;
    _state_var[_qp][n] = rho_i[i];
  }

  // Store backstress values SDV 50 + _num_slip_sys*3
  for (unsigned int i = 0; i < _num_slip_sys; i++) {
    n = n + 1;
    _state_var[_qp][n] = bstress[i];
  }

  // Store tau values SDV 50 + _num_slip_sys*4
  for (unsigned int i = 0; i < _num_slip_sys; i++) {
    n = n + 1;
    _state_var[_qp][n] = tau[i];
  }
...
void DDCPTSTStressUpdate::NR_residual (unsigned int num_slip_sys, std::vector<std::vector<Real>> &xs0, std::vector<std::vector<Real>> &xm0, Real temp, Real dt, std::vector<Real> gdt, RankTwoTensor F1, RankTwoTensor &F_el, RankTwoTensor &F_p_inv, RankTwoTensor F_p_inv_0, Real C[3][3][3][3], std::vector<Real> &rho_m0, std::vector<Real> &rho_m, std::vector<Real> &rho_i0, std::vector<Real> &rho_i, std::vector<Real> &bstress0, std::vector<Real> &bstress, RankTwoTensor &sig, std::vector<Real> &tau, std::vector<Real> &tau_eff, std::vector<Real> &s_a, std::vector<Real> &s_t, std::vector<std::vector<Real>> A, std::vector<std::vector<Real>> H, std::vector<Real> &residual, Real &sse){
  ...
  for(unsigned int k = 0; k < num_slip_sys; k++) {
    ...
    // if((abs(tau_eff[k]) < tau_c) && (abs(tau_eff[k]) > 0.5*tau_c)) {
    if(abs(tau_eff[k]) < tau_c) {
      shr_g[k] = refgdg*exp((-delF0/B_k/temp)*power(1.0 - power(abs(tau_eff[k])/tau_c, p), q))*sgn(tau_eff[k]);
    } else if(abs(tau_eff[k]) >= tau_c) {
      shr_g[k] = refgdg*sgn(tau_eff[k]);
    }
  }
...
void DDCPTSTStressUpdate::readPropsFile() {
...
void DDCPTSTStressUpdate::assignProperties(){
  ...
  // Flow parameters
  attempt_freq = _properties[_qp][9];  // attempt_freq
...
void DDCPTSTStressUpdate::normalize_vector(Real *x, Real *y, Real *z) {
...
void DDCPTSTStressUpdate::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void DDCPTSTStressUpdate::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
...
void DDCPTSTStressUpdate::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void DDCPTSTStressUpdate::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void DDCPTSTStressUpdate::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
void DDCPTSTStressUpdate::aa_dot_bb(Real a[3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real DDCPTSTStressUpdate::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void DDCPTSTStressUpdate::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real DDCPTSTStressUpdate::power(Real x, Real y) {
...
Real DDCPTSTStressUpdate::sgn(Real x) {
...
Real DDCPTSTStressUpdate::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
