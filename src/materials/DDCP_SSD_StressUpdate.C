#include "DDCP_SSD_StressUpdate.h"
...
registerMooseObject("RhocpApp", DDCP_SSD_StressUpdate);

InputParameters
DDCP_SSD_StressUpdate::validParams()
{
  ...
  params.addCoupledVar("tol", 1.0e-6, "Tolerance");
...
DDCP_SSD_StressUpdate::DDCP_SSD_StressUpdate(const InputParameters & parameters) :
...
DDCP_SSD_StressUpdate::~DDCP_SSD_StressUpdate()
{

}

void DDCP_SSD_StressUpdate::initQpStatefulProperties()
{
...
void DDCP_SSD_StressUpdate::computeQpStress()
{
  ...
  RankTwoTensor F_el, F_el_prev; //  Elastic part of F
  ...
  RankTwoTensor sig;
  RankTwoTensor Spk2;
  Real latticeStrain_old, latticeStrain_new, cosangle;
  ...
  if (_t_step <= 1) {
  // read in Euler angles from EBSD data
    if (_EBSDFileReader){
    ...
    // read in Euler angles for *.e data
  ...
  if(_t_step > 1) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
      for (unsigned int j = 0; j < 3; j++) {
        n = n + 1;
        F_el_prev(i,j) = _state_var[_qp][n];
...
/*  RankTwoTensor Ftheta, Ftheta_old;
  ...
  for (int i=0; i<3; ++i)
  {
      Ftheta(i,i) = sqrt(1.0 + 2.0 * _eigenstrain[_qp](i,i));
      Ftheta_old(i,i) = sqrt(1.0 + 2.0 * _eigenstrain_old[_qp](i,i));
  }
...
//       }
    ...
    std::vector<std::vector<Real>> k_delta(_num_slip_sys, std::vector<Real>(_num_slip_sys));
    std::vector<std::vector<Real>> dbsdgb(_num_slip_sys, std::vector<Real>(_num_slip_sys));
    std::vector<std::vector<Real>> dtaub_dgb_rs(_num_slip_sys, std::vector<Real>(_num_slip_sys));
...
/*    //backstress
    for (unsigned int ia = 0; ia < _num_slip_sys; ia++) {
      for (unsigned int ja = 0; ja < _num_slip_sys; ja++) {
        if (gnd_from_Nyetensor[ia] > 1) {
          dtaub_dgb_rs[ia][ja] = dt_incr * ((kin_hardening_const * G * b_mag * 0.5 * del_rhog_dgb[ia][ja] / sqrt(gnd_from_Nyetensor[ia])) - (kin_hardening_const * G * b_mag * (1/4) * power(gnd_from_Nyetensor[ia],-1.5) * gnd_rate_from_Nyetensor[ia] * del_rhog_dgb[ia][ja] * dt_incr * sgn(signed_gnd_from_Nyetensor[ia])));
        }
        else {
          dtaub_dgb_rs[ia][ja] = 0.0;
        }
      }
    }*/
...
//   }
  ...
  // Effective total strain SDV 44
  E_eff = sqrt((2./3.) * s1);

  // Effective plastic strain SDV 45
  E_p_eff = sqrt((2./3.) * s2);
  ...
  for (unsigned int ia = 0; ia < 3; ia++) {
    for (unsigned int ja = 0; ja < 3; ja++) {
      ...
      for (unsigned int ka = 0; ka < _num_slip_sys; ka++) {
        bstress_mean[ia][ja] = bstress_mean[ia][ja] + xs0[ia][ka] * xm0[ja][ka] * bstress[ka];
      }
    }
  }
...
void DDCP_SSD_StressUpdate::readPropsFile() {
...
void DDCP_SSD_StressUpdate::assignProperties(){
...
void DDCP_SSD_StressUpdate::normalize_vector(Real *x, Real *y, Real *z) {
...
void DDCP_SSD_StressUpdate::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void DDCP_SSD_StressUpdate::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
...
void DDCP_SSD_StressUpdate::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void DDCP_SSD_StressUpdate::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void DDCP_SSD_StressUpdate::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
void DDCP_SSD_StressUpdate::aa_dot_bb(Real a[3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real DDCP_SSD_StressUpdate::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void DDCP_SSD_StressUpdate::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real DDCP_SSD_StressUpdate::power(Real x, Real y) {
...
Real DDCP_SSD_StressUpdate::sgn(Real x) {
...
Real DDCP_SSD_StressUpdate::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
